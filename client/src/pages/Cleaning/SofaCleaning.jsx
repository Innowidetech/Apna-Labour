...
import leatherSofa from "../../assets/leather1.png";
...
const modalServices = [
  { id: 1, name: "3 Seater Sofa Cleaning", price: "₹749", discount: "10% Off" },
  { id: 2, name: "5 Seater Sofa Deep Cleaning", price: "₹1299", discount: "15% Off" },
  { id: 3, name: "Fabric Protection Coating", price: "₹499", discount: "5% Off" },
];

const otherServices = [
  { id: 1, name: "Curtain Cleaning", price: "₹499", rating: "4.5", reviews: "600", img: other1 },
    ...
    <>
      ...
      <section className="bg-[#EDF2FB] py-6 px-6 md:px-16">
        <div className="container mx-auto flex flex-col md:flex-row justify-between">
          <div className="w-full md:w-1/2 text-left flex flex-col justify-end">
            <h2 className="text-2xl font-bold text-gray-800">Sofa Cleaning Services</h2>
            <p className="text-gray-600 mt-2">
              Professional fabric and leather sofa cleaning with deep shampooing and conditioning.
            </p>
          </div>
          <div className="w-full md:w-1/2 flex justify-end items-end">
            <img src={manImg} alt="Sofa Cleaning" className="max-w-xs md:max-w-sm lg:max-w-md self-end" />
          </div>
        </div>
      </section>
      ...
      <section className="py-10 px-6 md:px-16">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
          <div
            onClick={() => setSelectedService({ name: "Sofa" })}
            className="cursor-pointer p-4 flex flex-col items-center"
          >
            <img src={sofaImg} alt="Sofa" className="w-44 h-32 object-contain p-4 border border-gray-300" />
            <p className="mt-3 font-medium text-[#023E8A] text-center">Sofa</p>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 px-6 md:px-16 bg-white">
        <h3 className="text-xl font-semibold mb-8">Popular Sofa Cleaning Services</h3>
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
          {sofaServices.map((service) => (
            <div key={service.id} className="rounded-xl border border-gray-200 shadow-sm bg-white overflow-hidden hover:shadow-md transition">
      ...
      {selectedService && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white rounded-2xl shadow-lg p-6 w-[95%] md:w-[900px] relative">
            ...
            <p className="text-sm text-gray-500 mb-4">⭐ 4.8 (3.1k bookings)</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
              {modalServices.map((srv) => (
                <div key={srv.id} className="p-4 border rounded-xl shadow-sm hover:shadow-md flex flex-col items-center text-center">
                  ...
                  <button className="mt-3 px-4 py-2 bg-[#86A8E7] text-white text-sm rounded-lg hover:bg-blue-700">Add</button>
                </div>
              ))}
            </div>
          </div>

...
// Placeholder for repairMan image
const repairMan = "https://placehold.co/100x100/A0B9D6/white?text=Worker";
...
            // Using placeholder if image is missing or cannot be resolved
            image: data.image || "https://placehold.co/400x224/EBF4FF/274D7D?text=Service+Banner",
          });
        } else {
          setHeroData({ title: "Service", image: "https://placehold.co/400x224/EBF4FF/274D7D?text=Service+Banner" });
        }
      } catch {
        setHeroData({ title: "Service", image: "https://placehold.co/400x224/EBF4FF/274D7D?text=Service+Banner" });
      }
...
      // NOTE: Replaced alert with console log/custom message as per guidelines
      console.error("Please fill all fields before booking."); 
      return;
    }
...
    ) {
      // NOTE: Replaced alert with console log/custom message as per guidelines
      console.error("Please fill all fields before booking team.");
      return;
    }
...
      const diff = e - s;
      // Calculate days including start and end date (+1)
...
            <img
              src={heroData.image || "https://placehold.co/400x224/EBF4FF/274D7D?text=Service+Banner"}
              alt={heroData.title}
...
    {/* Labour Details Modal (Updated to use the better structure) */}
{isLabourDetailsModalOpen && labourDetails && (
  <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
    <div className="bg-white p-6 rounded-2xl shadow-xl w-11/12 max-w-md relative max-h-[85vh] overflow-y-auto">
      <button
...
            {labourDetails?.subject || labourDetails?.name || "Worker"}
            {/* Using name as a fallback for subject in case API varies */}
          </h2>
...
              {labourDetails?.averageRating ?? 0} ({labourDetails?.totalReviews || 0} reviews)
            </p>
          </div>
...
              <h3 className="font-semibold mb-2 text-left w-full">Reviews:</h3>
              <ul className="space-y-2 w-full">
                {labourDetails.reviews.map((r, index) => (
                  <li key={index} className="border-b pb-1">
                    <p className="text-sm font-medium">{r.user || `User ${index + 1}`}</p>

...
const ServiceDetails = () => {
  ...
  // Fetch detailed info about a particular service
  ...
  // Fetch labourers by type ('Team' or 'Individual')
  const fetchCasualLabour = async (type) => {
    try {
      ...
      // Optionally apply search and location filters client-side here if needed:
      let filteredLabourers = data.labourers || [];
      if (searchTerm.trim()) {
        filteredLabourers = filteredLabourers.filter((labour) =>
          labour.name.toLowerCase().includes(searchTerm.toLowerCase())
        );
      }
      if (locationFilter.trim()) {
        filteredLabourers = filteredLabourers.filter((labour) =>
          labour.distance.toLowerCase().includes(locationFilter.toLowerCase())
        );
      }

      setCasualLabour(filteredLabourers);
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="px-6 md:px-16 py-6">
        ...
        {/* Casual Labour Section - Updated layout */}
        <section className="mb-8">
          <div className="flex flex-col md:flex-row justify-between items-start gap-6">
            ...
            <div className="flex flex-col space-y-4 md:w-1/3">
              ...
              <input
                type="text"
                placeholder="Filter by location/distance"
                value={locationFilter}
                onChange={(e) => setLocationFilter(e.target.value)}
                className="border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-600"
              />

              {/* You can add a button to apply filters or auto-apply on typing as above */}
              <button
                onClick={() => {
                  // refetch with current filters applied
                  // For now, just refetch current type or no-op
                  // You could store last type clicked or default to Individual
                  fetchCasualLabour("Individual");
                }}
            ...
            {/* Right Side: Team & Individual Buttons + Labour List */}
            <div className="md:w-2/3">
              ...
              {!isLoadingCasualLabour &&
                ...
                !errorCasualLabour && <p>No labourers found for selected type.</p>}
            </div>
          </div>
        </section>

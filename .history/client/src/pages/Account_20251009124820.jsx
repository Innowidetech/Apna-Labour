...
const Account = () => {
  ...
  useEffect(() => {
    if (!accountData) {
      console.log("No accountData yet");
      return;
    }
    console.log("Populating formData from accountData:", accountData);
    ...
    setFormData((prev) => ({
      ...
      address: {
        ...
        buildingName: accountData.address?.buildingName || "", // New
      ...
      image: null,
      imageUrl: accountData.image || "",  // Use API's "image" field
    }));

    setDisplayName(
      accountData.name || accountData.email || accountData.phoneNumber || "User"
    );
  }, [accountData]);
  ...
  const handleSave = async () => {
    try {
      ...
      Object.entries(formData.address).forEach(([key, value]) => {
        dataToSend.append(`address.${key}`, value);
      });
      ...
      await dispatch(updateAccountProfile(dataToSend)).unwrap();

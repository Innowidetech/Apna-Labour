...
const ServiceDetails = ({ casualLabourMode = false }) => {
  const { id } = useParams();
  ...
  const [labourType, setLabourType] = useState("Team"); // default filter
  const [searchTerm, setSearchTerm] = useState("");

  // Fetch subcategories and hero data on mount or when id changes
  useEffect(() => {
    if (!casualLabourMode) {
      // Existing fetches when NOT casual labour mode
      ...
      // Casual labour hero fetch only (if needed)
      const fetchHero = async () => {
        try {
          const res = await fetch(
            `https://apnalabour.onrender.com/api/customer/hero/${id}`
          ...
          if (res.ok) {
            setHeroData({
              title: data.title || "Casual Labour",
              image: data.image || "",
            });
          } else {
            setHeroData({ title: "Casual Labour", image: "" });
          }
        } catch {
          setHeroData({ title: "Casual Labour", image: "" });
        }
      ...
      // Fetch casual labour for default labourType
      fetchCasualLabour(labourType);
    }
  }, [id, casualLabourMode]);

  // Fetch casual labour by type (Team or Individual)
  ...
  // Handle labourType change in casual labour mode
  const handleLabourTypeChange = (type) => {
    setLabourType(type);
    fetchCasualLabour(type);
  };

  // Filter casual labour based on search term
  const filteredCasualLabour = casualLabour.filter((labour) =>
    labour.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Existing fetchServices, fetchServiceDetail etc. remain unchanged

  // Existing handleSubcategoryClick remains unchanged
  ...
  return (
    <>
        ...
        <div className="container mx-auto flex flex-col md:flex-row justify-between items-end">
          <div className="w-full md:w-1/2 text-left">
            <h2 className="text-2xl font-bold text-gray-800">
              {heroData.title || (casualLabourMode ? "Casual Labour" : "Service")}
            </h2>
          </div>
      ...
      <div className="px-6 md:px-16 py-6">
        {casualLabourMode ? (
          <>
            {/* Casual Labour Controls on right */}
            <div className="flex justify-end items-center gap-4 mb-6">
              <div className="flex gap-2">
                <button
                  ...
                  onClick={() => handleLabourTypeChange("Team")}
                >
                  Team
                </button>
                <button
                  ...
                  onClick={() => handleLabourTypeChange("Individual")}
                >
                  Individual
                </button>
              </div>
            ...
            {!isLoadingCasualLabour && filteredCasualLabour.length === 0 && (
              <p>No labourers found.</p>
            )}

            {!isLoadingCasualLabour && filteredCasualLabour.length > 0 && (
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {filteredCasualLabour.map((labour) => (
                  <div
                    key={labour._id}
                  ...
                  >
                    ...
                    <p className="text-sm text-gray-600 mb-1">
                      ...
                      {labour.totalReviews || 0} reviews)
                    </p>
                    <p className="text-sm text-gray-500">📞 {labour.mobileNumber}</p>
                    <p className="text-sm text-gray-500">Distance: {labour.distance}</p>
                    <p className="text-sm text-gray-500">
                      Available: {labour.isAvailable ? "Yes" : "No"}
                    </p>
                  </div>
                ))}
              </div>
          ...
          <>
            {/* Existing Subcategories Section */}

...
const Account = () => {
  ...
  // Update form and sidebar when accountData changes
  useEffect(() => {
  ...
  // Manage image preview URL for performance
  useEffect(() => {
    if (displayData.image && typeof displayData.image !== "string") {
      const url = URL.createObjectURL(displayData.image);
  ...
  const handleSave = async () => {
    try {
      ...
      // Fetch updated profile to reflect persistent data
      await dispatch(fetchAccountProfile());

      // Immediately update display name
      setDisplayData((prev) => ({
        ...prev,
        name: formData.name,
      }));
  ...
  const handleLogout = () => {
    localStorage.removeItem("token"); // <-- fixed key here

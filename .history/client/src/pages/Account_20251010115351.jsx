...
const Account = () => {
  ...
  useEffect(() => {
    if (!accountData || !accountData.profile) return;

    // According to backend, profile keys are at root of profile object
    // We merged user and customer data in backend in profile object
    ...
    const newFormData = {
      ...
      image: null, // always null initially for file input
    };
    ...
    setDisplayData({
      ...
      image: profile.image || profile.picture || null, // use customer.image or user.picture
    });
  }, [accountData]);
  ...
  const handleLogout = () => {
    localStorage.removeItem("token"); // fix token key

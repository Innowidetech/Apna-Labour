...
const ServiceDetails = () => {
  ...
  try {
    const res = await fetch(`https://apnalabour.onrender.com/api/customer/hero/${id}`);

    if (res.status === 404) {
      // Fallback when hero not found
      setHeroData({
      ...
      <div className="px-6 md:px-16 py-6">
        ...
        {heroData.title === "Casual Labour" && (
          <div className="px-6 md:px-16 py-6">
            ...
            {isLoadingLabour && <p>Loading {labourType}...</p>}
            {errorLabour && <p className="text-red-500">{errorLabour}</p>}
            {!isLoadingLabour &&
              labourData.length === 0 &&
              !errorLabour &&
              labourType && <p>No {labourType} available.</p>}

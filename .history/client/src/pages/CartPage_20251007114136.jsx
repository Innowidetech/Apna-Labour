...
// --- Slot Booking ---
 ...
 const handleFetchProfile = async () => { try { const token = localStorage.getItem("token"); 
if (!token) { toast.error("Please login first!");
 return; } setProfileLoading(true); 
 const res = await fetch("https://apnalabour.onrender.com/api/customer/profile/name", { method: "GET", headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}`, }, }); const data = await res.json(); if (!res.ok) throw new Error(data.message || `HTTP ${res.status}`); setProfileData(data); toast.success("Profile details fetched successfully!"); } catch (err) { toast.error(err.message || "Failed to fetch profile details"); } finally { setProfileLoading(false); } };
  return (

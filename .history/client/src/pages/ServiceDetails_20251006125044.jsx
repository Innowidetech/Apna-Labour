...
const ServiceDetails = () => {
  ...
  // Fetch services for a subcategory or casual labour type
  const fetchServices = async (subcategoryId) => {
    try {
      ...
      if (isCasualLabour) {
        // Casual labour API: subcategoryId = Team / Individual
        const res = await fetch(
        ...
        if (!Array.isArray(data) || data.length === 0) {
          setErrorServices("No labourers found for this type.");
        } else {
          setServices(data);
        }
      } else {
  ...
  const fetchServiceDetail = async (serviceId) => {
    try {
      ...
      if (isCasualLabour) {
        // For casual labour, no separate service detail API, open modal with clicked labourer
        ...
        {selectedSubcategoryId && (
          <section>
            <h3 className="text-xl font-semibold mb-4">
              {isCasualLabour ? "Labourers" : "Services"}
            </h3>
              ...
              {services.map((service) => (
                ...
                >
                  <img
                    ...
                    alt={service.title || service.name || service.type}
                    className="w-20 h-20 object-contain mx-auto"
                  />
                  <p className="mt-2 text-sm font-medium">
                    {service.title || service.name || service.type || service.name}
                  </p>
                  {service.price && (
                ...
                {Array.isArray(serviceDetail) ? (
                  ...
                  <div className="w-64 p-4 border rounded-lg shadow">
                    <h2 className="text-lg font-bold mb-2">
                      {serviceDetail.title || serviceDetail.name || serviceDetail.type}
                    </h2>
                    {serviceDetail.image && (
                      <img

...
const CartPage = () => {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col lg:flex-row p-4 lg:p-10 relative">
      {/* Left Section */}
      {/* Left Section */} <div className="w-full lg:w-1/2 flex flex-col items-center pt-10 pb-6 px-4 lg:px-10"> <h1 className="text-2xl font-cursive mb-6">Apna Labour</h1> {!showLoginForm ? ( <div className="p-2 w-full max-w-xl text-left flex items-center gap-2 flex-wrap border border-gray-200 rounded-lg"> <p className="mb-0 text-base flex-1"> Login to book the service from your existing bag. </p> <button className="bg-black text-white px-4 py-2 rounded-md" onClick={() => setShowLoginForm(true)} > LOGIN NOW </button> </div> ) : ( <div className="p-6 w-full max-w-sm"> {step === "login" && ( <> <h2 className="text-lg mb-6 text-left">Login / Signup</h2> <input type="text" placeholder="Mobile Number" value={emailMobile} onChange={(e) => setEmailMobile(e.target.value)} className="border-b border-gray-300 px-3 py-2 focus:outline-none focus:border-blue-500 w-full" /> <p className="text-sm mt-4"> By Continuing you’ve agree to the Terms of Services & Privacy Policy. </p> <button className="bg-black text-white px-4 py-2 rounded-md w-full mt-4" onClick={handleLogin} > Send OTP </button> </> )} {step === "otp" && ( <> <h2 className="text-lg mb-6 text-left">Verify OTP</h2> <input type="text" placeholder="Enter OTP" value={otp} onChange={(e) => setOtp(e.target.value)} className="border-b border-gray-300 px-3 py-2 focus:outline-none focus:border-blue-500 w-full" /> <button className="bg-black text-white px-4 py-2 rounded-md w-full mt-4" onClick={handleVerifyOtp} > Verify OTP </button> </> )} {step === "next" && ( <> <h2 className="text-lg mb-6 text-left">Complete Booking</h2> <div className="flex flex-col gap-4"> <button className="bg-blue-600 text-white px-4 py-2 rounded-md" onClick={() => setAddressModalOpen(true)} > {addressSaved ? "Edit Address" : "Register Address"} </button> <button className="bg-gray-100 px-4 py-2 rounded-md text-left cursor-default"> 📅 Slot Booking (Coming Soon) </button> <button disabled={!addressSaved} className={`px-4 py-2 rounded-md mt-4 ${ addressSaved ? "bg-green-600 text-white" : "bg-gray-200 text-gray-500 cursor-not-allowed" }`} > Proceed to Payment </button> </div> </> )} </div> )} {/* Cancellation Policy */} <p className="mt-6 text-sm text-gray-700"> <span className="font-bold">Cancellation Policy</span> <br /> Free cancellation if done more than 12hrs before the service or a professional isn’t assigned. A fee will be charged otherwise. </p> </div>

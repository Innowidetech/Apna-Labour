...
const AccountSidebar = ({ handleLogout, handleDeactivate }) => {
  ...
  // ✅ Filter bookings by upcoming/past or all
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 w-64 bg-gray-100 border-r overflow-y-auto transform
          transition-transform duration-300
          ${isOpen ? "translate-x-0" : "-translate-x-full"}
          md:translate-x-0 md:static md:inset-auto md:flex-shrink-0`}
      >
        {/* Profile */}
        <div className="p-4 flex flex-col items-center text-center md:mt-4">
      ...
      {/* Mobile Overlay */}
      {isOpen && <div className="fixed inset-0 bg-black opacity-25 z-40 md:hidden" onClick={() => setIsOpen(false)} />}
    </>
  );
};

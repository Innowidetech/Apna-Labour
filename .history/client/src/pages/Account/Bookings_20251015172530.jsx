...
const Bookings = () => {
  ...
  const [reviewModal, setReviewModal] = useState({ open: false, bookingId: null, itemId: null });
  ...
  const handleReviewClick = (bookingId, itemId) => {
    setReviewModal({ open: true, bookingId, itemId });
    setReviewRating(0);
    setReviewFeedback("");
  };
...
const handleReviewSubmit = () => {
  if (!reviewRating || !reviewModal.itemId) return; // ensure itemId exists

  dispatch(addReview({
    unitId: reviewModal.itemId,
    rating: reviewRating,
    feedback: reviewFeedback,
  }))
    .unwrap()
    .then(() => {
      setReviewModal({ open: false, bookingId: null, itemId: null });
      setReviewRating(0);
      setReviewFeedback("");
          ...
          <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => { setSelectedBookingId(null); setCancelReason(""); }}
                ...
                >{cancelLoading ? "Cancelling..." : "Confirm Cancel"}</button>
              </div>
            </div>
          </div>
        )}
        ...
        {reviewModal.open && (
          <div className="fixed inset-0 flex items-center justify-center bg-black/40 z-50">
            <div className="bg-white rounded-xl p-6 w-full max-w-md">
              ...
              <div className="flex justify-end gap-3">
                <button
                  onClick={() => setReviewModal({ open: false, bookingId: null, itemId: null })}
                  className="px-4 py-2 rounded-lg border hover:bg-gray-100"
                >Close</button>
                <button
                ...
                >{reviewLoading ? "Submitting..." : "Submit"}</button>
              </div>
            </div>
          </div>
        )}
    ...
    <div className="bg-white rounded-2xl shadow-md p-5 border border-gray-100 mb-6">
      ...
      <div className="space-y-4">
        {booking.items?.map((item, index) => (
          <div
            key={`${booking._id}-${index}`} // unique key fix
            ...
            <div className="flex flex-col items-end gap-1">
              ...
              {onReview && (
                <button
                  ...
                  className="px-3 py-1 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700 mt-1"
                >
                  Review
                </button>
              )}

...
import {
  ...
  cancelBooking,
  addReview,
} from "../../redux/bookingSlice"; // make sure addReview is implemented in your slice
...
const Bookings = () => {
  ...
  // Review state for specific item
  const [reviewItem, setReviewItem] = useState(null); // { bookingId, itemId }
  ...
  const handleReviewSubmit = () => {
    ...
    dispatch(
      addReview({ unitId: reviewItem.itemId, rating, feedback })
    )
      .unwrap()
      .then(() => {
        setReviewItem(null);
  ...
  return (
    <div className="flex flex-col md:flex-row bg-gray-50 min-h-screen">
      ...
      <div className="flex-1 p-4 md:p-8">
        ...
        {/* Upcoming */}
        <BookingList bookings={upcoming} onCancel={handleCancelClick} />

        {/* Past */}
        <BookingList
          bookings={past}
          onReview={(bookingId, itemId) =>
            setReviewItem({ bookingId, itemId })
          }
        />
...
// Booking list
const BookingList = ({ bookings, onCancel, onReview }) => {
        ...
        {booking.items?.map((item) => {
          ...
          return (
            <div
              ...
              className="flex flex-col md:flex-row items-center justify-between border border-gray-100 rounded-xl p-3 hover:bg-gray-50 transition"
            >
              ...
              <div className="flex flex-col items-end">
                <span className="font-semibold text-gray-700 mt-2 md:mt-0">
                  ₹{total.toFixed(2)}
                </span>

                {booking.status === "Completed" && onReview && (
                  <button
                    onClick={() => onReview(booking._id, item._id)}
                    className="px-3 py-1 mt-2 rounded-lg bg-blue-600 text-white text-sm hover:bg-blue-700"
                  >
                    Review
                  </button>
                )}

                {/* Display stars if already reviewed */}
                {item.rating && (

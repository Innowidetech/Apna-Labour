  ...
  return (
    <>
      {/* Mobile Top Bar */}
       ...
       {/* Mobile Top Bar */}
<div className="md:hidden flex items-center bg-gray-100 p-4 border-b">
  {/* Toggle button at left */}
  <button
  ...
  <div className="flex items-center space-x-2">
    {image ? (
      ...
      <span className="w-10 h-10 rounded-full bg-gray-300 flex items-center justify-center text-white font-bold">
        {name.charAt(0).toUpperCase()}
      </span>
    )}
    <span className="font-bold text-lg">{name}</span>
      ...
      >
        ...
        <div className="flex flex-col gap-2 mt-6 px-6 pb-6">
          {handleLogout && (
            <button className="w-full bg-red-500 text-white py-2 rounded-md hover:bg-red-600 transition duration-200" onClick={handleLogout}>
              Logout
            </button>
          )}
          {handleDeactivate && (
            <button className="w-full bg-gray-700 text-white py-2 rounded-md hover:bg-gray-800 transition duration-200" onClick={handleDeactivate}>
              Deactivate Account
            </button>
          )}
        </div>

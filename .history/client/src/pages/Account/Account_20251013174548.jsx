...
const Account = () => {
  ...
  const [confirmAction, setConfirmAction] = useState(null); // 🟢 track pending confirm type
  ...
  // Handle confirmation toast logic
  const handleConfirmAction = (type, actionFn) => {
    if (confirmAction === type) {
      // ✅ Second click within timeout → execute action
      actionFn();
      setConfirmAction(null);
    } else {
      // ⚠️ First click → show confirmation toast
      setConfirmAction(type);
      toast.warning(`Click again to confirm ${type === "delete" ? "deletion" : "deactivation"}`, {
        autoClose: 5000,
      ...
      <div className="flex min-h-screen bg-gray-50">
        ...
        <div className="flex-1 p-6 md:p-10 bg-white">
          ...
          <div className="flex justify-end mb-4">
            ...
            >
              {isEditable ? "Cancel editing" : "Change profile information"}
            </button>
          </div>
          ...
          <div className="max-w-3xl mx-auto space-y-5">
            ...
            <div className="grid grid-cols-3 gap-3 mt-3">
              {["pincode", "townCity", "state"].map((field) => (
                <input
                  key={field}
                  type="text"
                  name={`address.${field}`}
            ...
            <div className="text-center mt-4 space-y-1">
              <button
                onClick={() => handleConfirmAction("deactivate", handleDeactivate)}
                className="text-sm text-blue-600 hover:underline block"
              >
                Deactivate account
              </button>
              <button
                onClick={() => handleConfirmAction("delete", handleDelete)}

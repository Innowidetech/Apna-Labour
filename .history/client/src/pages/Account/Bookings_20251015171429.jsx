...
const Bookings = () => {
  ...
  const handleReviewSubmit = (bookingId) => {
    ...
    const unitId = booking?.unitId || booking?._id; // Use unitId or bookingId if unitId is not present

    if (!unitId) {
      console.error("Unit ID is missing for the review");
      return;
    }

    if (rating === 0 || feedback === "") return; // Ensure both rating and feedback are provided

    dispatch(addReview({ unitId, rating, feedback }))
      .unwrap()
      .then(() => {

...
const Bookings = () => {
  ...
  const { bookings, upcoming, past, loading, error } = useSelector((state) => state.bookings);
  const [filter, setFilter] = useState("all"); // all | last30 | 2025 etc.
    ...
    <div className="flex flex-col md:flex-row bg-gray-50 min-h-screen">
      ...
      <div className="flex-1 p-4 md:p-8">
        ...
        <div className="flex gap-3 mb-6">
          {["all", "last30", String(new Date().getFullYear())].map((f) => (
            <button
              key={f}
              ...
              onClick={() => setFilter(f)}
            >
              {f === "all" ? "All" : f === "last30" ? "Last 30 Days" : f}
            </button>
          ))}
        </div>
        ...
        {/* For filters that don't separate upcoming/past */}
        {!upcoming && !past && bookings && bookings.length === 0 && (

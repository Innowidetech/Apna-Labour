...
const Account = () => {
  ...
  // Separate state for sidebar display to avoid live updates
  // image can be either a File object or a URL string
  ...
  useEffect(() => {
    ...
    const newFormData = {
      ...
      image: null, // always null initially for form upload input
    };
    ...
    setDisplayData({
      ...
      // Use existing image URL if available (adjust key as per your backend)
      image: customer.imageUrl || null,
    });
  }, [accountData]);
  ...
  const handleImageChange = (e) => {
    ...
    // Update sidebar image preview immediately when user selects a new image
    if (file) {
  ...
  const handleSave = async () => {
    try {
      ...
      // After fetch, the profile image URL will update in displayData via useEffect
      // but to be safe, update sidebar name now
      setDisplayData((prev) => ({
        ...prev,
        name: formData.name,
      }));
        ...
        <div className="w-full md:w-64 bg-gray-100 flex-shrink-0 ml-2">
          <div className="p-6 flex flex-col items-center">
            <div className="w-20 h-20 bg-gray-300 rounded-full mb-4 flex items-center justify-center overflow-hidden">
              {displayData.image ? (
                // Show image preview correctly:
                // If image is a File object, use URL.createObjectURL

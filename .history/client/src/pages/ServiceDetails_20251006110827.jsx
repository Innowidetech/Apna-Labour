...
const ServiceDetails = () => {
  ...
  const [labours, setLabours] = useState([]); // 👈 new state for casual labour
  ...
  const handleSubcategoryClick = (subcategoryId, subTitle) => {
    ...
    setServices([]);
    setLabours([]);
    // 👇 If subcategory is "Casual Labour", use the new API
    if (subTitle?.toLowerCase().includes("labour")) {
      fetchCasualLabour("Individual"); // or "Team" depending on design
    } else {
  ...
  return (
    <>
      ...
      <div className="px-6 md:px-16 py-6">
        ...
        {errorLabours && <p className="text-red-500 mt-4">{errorLabours}</p>}
      </div>
    </>
  );
};

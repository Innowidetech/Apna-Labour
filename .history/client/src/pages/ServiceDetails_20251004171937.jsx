...
const ServiceDetails = () => {
  ...
  // Replace this with your actual Casual Labour category ID
  const CASUAL_LABOUR_ID = "66f2d9cda91b2e879c45aa99";
  ...
  useEffect(() => {
    // Redirect to Casual Labour page if the category is Casual Labour
   
    if (id === CASUAL_LABOUR_ID) {
    navigate(`/casual-labour/${id}`);
    return;
  }
  fetchSubcategories();
  fetchHeroData();
  ...
  try {
    ...
    if (!res.ok) {
      // Fallback for missing hero
      setHeroData({
        title: "Service",
        image: repairMan, // default image
      });
      return;
    }
  ...
  const fetchServices = async (subcategoryId) => {
    try {
      ...
      setErrorServices(err.message || "Something went wrong while fetching services.");
    } finally {
  ...
  const fetchServiceDetail = async (serviceId, categoryId) => {
    // If this service is Casual Labour, redirect to Casual Labour page
    if (categoryId === CASUAL_LABOUR_ID) {
      navigate(`/casual-labour/${categoryId}`);
      return;
    }
    ...
    try {
      ...
      setErrorServiceDetail(err.message || "Something went wrong while fetching service detail.");
    } finally {
  ...
  return (
    <>
      ...
      <div className="px-6 md:px-16 py-6">
        ...
        {errorSubcategories && <div className="text-red-500">{errorSubcategories}</div>}
        {!isLoadingSubcategories && subcategories.length === 0 && <div>No subcategories found.</div>}
        ...
        <div className="flex flex-col sm:flex-row justify-start gap-6 sm:gap-10 mb-8 px-6 md:px-16">
          {subcategories.map((sub) => (
            ...
            >
              <img src={sub.image || repairMan} alt={sub.title} className="w-full h-full object-cover" />
            </div>
          ))}
        </div>
        ...
        {selectedSubcategoryId && (
          <section>
            ...
            {isLoadingServices && <p className="text-gray-500">Loading services...</p>}
            {errorServices && <p className="text-red-500">{errorServices}</p>}
            {!isLoadingServices && services.length === 0 && !errorServices && <p>No services found.</p>}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6">
              {services.map((service) => (
                <div
                  ...
                  onClick={() => fetchServiceDetail(service._id, service.categoryId)}
                >
                  <img
      ...
      {isModalOpen && serviceDetail && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-white rounded-lg shadow-xl p-6 w-11/12 max-w-5xl relative">
            ...
            {errorServiceDetail && <p className="text-red-500">{errorServiceDetail}</p>}

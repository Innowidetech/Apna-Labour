...
const CartPage = () => {
  ...
  try {
    ...
    const res = await fetch("https://apnalabour.onrender.com/api/customer/bookings/create", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${token}`,
      ...
      body: JSON.stringify({
        paymentMethod: "COD", // Cash on Delivery (COD) as default payment method
        tip: 50, // Fixed tip value, can be dynamically adjusted if required
      }),
    });
    ...
    // Optionally, reset any states or redirect the user if needed
    setCartItems([]);
    setTotalPrice(0);
    setAddressSaved(false); // Reset address save state if needed
    setSlotBooked(false); // Reset slot booking state
  } catch (err) {
...
<>
  <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 w-full max-w-md mx-auto">
    ...
    <div className="border-b border-gray-100 py-4">
      <div className="flex items-center gap-2 mb-2">
        <span className="text-gray-500 text-lg bg-[#003049]">📅</span>
        <h3 className="text-gray-800 text-sm font-medium">Slot booking</h3>
      </div>
      <button
        className="w-full bg-gray-200 text-gray-400 py-2 rounded-md font-medium text-sm transition-colors active:bg-[#003049] active:text-white"
    ...
    <div className="py-4">
      ...
      {addressSaved ? (
        <button
          className="w-full bg-green-600 text-white py-2 rounded-md font-medium text-sm transition-colors hover:bg-green-700 active:bg-[#003049]"
        ...
        <button
          className="w-full bg-gray-200 text-gray-400 py-2 rounded-md font-medium text-sm transition-colors active:bg-[#003049] active:text-white bg-[#003049]"
        >
          Proceed to checkout
        </button>
      )}

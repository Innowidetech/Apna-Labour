...
const LabourCart = () => {
  ...
  const handleRemove = (id, startDate) => {
    const updatedCart = cartItems.filter(
      (item) => item.labourer._id + item.startDate !== id + startDate
    );
  ...
  const tax = 0.1 * (getGrandTotal() + serviceFee); // 10% tax
      ...
      <div className="flex-1 space-y-6 bg-white p-6 shadow-lg rounded-xl">
        ...
        {cartItems.length === 0 ? (
          ...
          cartItems.map((item, index) => (
            <div
              key={`${item.labourer._id}-${item.startDate}-${index}`} // unique key
            ...
            >
              ...
              <div>
                <button
                  ...
                  onClick={() => handleRemove(item.labourer._id, item.startDate)}
                >
                  Remove
                </button>
              </div>

...
const CasualLabour = () => {
  ...
  const fetchLabourers = async (type) => {
    try {
      setIsLoading(true);
      setError(null);
      setLabourers([]); // reset previous results
  ...
  // Fetch labourers whenever type changes
  useEffect(() => {
    ...
    <div className="min-h-screen bg-[#f9fafc]">
      ...
      <section className="bg-[#EDF2FB] py-6 px-6 md:px-16 flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Home / Casual Labourer / Request {labourType}
          </p>
        </div>
        <img
          src={workerImg}
      ...
      <div className="px-6 md:px-16">
        ...
        {!isLoading && labourers.length === 0 && !error && (
          <p className="text-gray-500">No {labourType} available currently.</p>
        )}
          ...
          {labourers.map((item, index) => (
            ...
            >
              ...
              <p className="text-sm text-gray-600">
                {item.skills?.join(", ") || "Lifting, Packing"}
              </p>

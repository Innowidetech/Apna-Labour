...
export const updateAccountProfile = createAsyncThunk(
  ...
  async (updatedData, { rejectWithValue }) => {
    try {
      ...
      // ✅ Send address as a JSON string to avoid invalid keys
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAccountProfile.fulfilled, (state, action) => {
        state.loading = false;
        state.accountData = action.payload?.customer || action.payload; // supports both API formats
      })
      ...
      .addCase(updateAccountProfile.fulfilled, (state, action) => {
        state.loading = false;
        state.accountData = action.payload?.customer || action.payload;

...
export const fetchAccountProfile = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      // Response contains { message, profile }
      return response.data.data; // or response.data.user, etc.
...
export const updateAccountProfile = createAsyncThunk(
  ...
  async (formData, { rejectWithValue }) => {
    try {
      // formData is expected to be FormData instance to support image upload
      const config = {
        headers: {
          ...getAuthHeaders(),
